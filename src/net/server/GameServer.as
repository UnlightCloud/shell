...
public class GameServer extends Server {
    ...
    private var _command:GameCommand;
    private var _crypted_sign:String;
    private var _ctrl:DuelCtrl;
    private var _match:Match;
    private var _duel:Duel;

    private static var __instance:GameServer;

    // 初期化 サーバを取得
    public function GameServer(caller:Function = null) {
        if (caller != createInstance) throw new ArgumentError("Cannot user access constructor.");
        _command = new GameCommand(this);
...
//            ActionCard.setLoaderFunc(getActionCardInfo);
    ...
    private static function createInstance():GameServer {
        return new GameServer(arguments.callee);
    }
    ...
    // 開始
    public function start(ctrl:DuelCtrl):void {
        _ctrl = ctrl;
    ...
    // オーバライド前提のGame専用HeatBeatHandler
    ...
    public function matchStart(uid:String):void {
        log.writeLog(log.LV_INFO, this, "Match Start ");
        host.sendCommand(_command.csMatchStart(uid));
    }

    private function checkMouseOperation():void {
        CONFIG::DUEL_NO_OP_CLOSE_ON
        {
            log.writeLog(log.LV_INFO, this, "checkMouseOperation !!");
            if (Unlight.INS.opetationCheck(DUEL_NO_OP_CLOSE_TIME)) {
                Alerter.forceOff();
                Alerter.showWithSize(_DISSCONET_NO_OPERATION, _TRANS_ALERT, 4, null, Alerter.reloadWindow, 110);
                host.close();
                MatchCtrl.instance.exit();
            } else if (Unlight.INS.opetationCheck(DUEL_NO_OP_ALERT_TIME)) {
                Alerter.showWithSize(_DISSCONET_NO_OP_ALERT, _TRANS_ALERT, 4, null, null, 110);
            }
        }
    }
...
//             _ctrl.setChatMessage(msg);
    ...
    public function scDetermineSession(id:int, foe:String, playerCharaID:String, foeCharaID:String, startDialogue:String, startDialogueId:int):void {
        log.writeLog(log.LV_INFO, this, "Determine GameSession", id, foe, "plccid:", playerCharaID, "foeccid:", foeCharaID, "starDialogue:", startDialogue, "starDialogueId:", startDialogueId);
        _ctrl.commandClear();
        ...
        _duel.initialize(string2intArray(playerCharaID), string2intArray(foeCharaID), 0, null, null, false);
    }
...
//                log.writeLog(log.LV_FATAL, this, dirs);
    ...
    public function scDuelDetermineMovePhase(init:Boolean, dist:int, list:String, dir:String, foeList:String, foeDir:String, pow:int, foePow:int, locked:Boolean, foeLocked:Boolean):void {
        ...
        log.writeLog(log.LV_INFO, this, "scDuelDetermineMovePhase ============== dist = " + dist);
    ...
    public function scDuelDeffenceCardDropPhaseFinish(list:String, dir:String, foeList:String, foeDir:String, locked:Boolean, foeLocked:Boolean):void {
        ...
        var playerCards:Array = ActionCard.idListToActionCards(string2intArray(list));
        var foeCards:Array = ActionCard.idListToActionCards(string2intArray(foeList));
        log.writeLog(log.LV_DEBUG, this, "::::::::::::::::::: Deffence Card Drop Phase Finish scHandler, locked?", locked, "foeLocked?", foeLocked);
        commandPush(_duel.foeEntrant.openBattleCards, [foeCards, string2BooleanArray(foeDir), foeLocked]);
...
//                _duel.plEntrant.charaChangeSuccess(index, cardId);
    ...
    public function scEntrantPartyDamagedEvent(player:Boolean, index:int, damage:int, is_not_hostile:Boolean):void {
        log.writeLog(log.LV_INFO, this, "Party Damage Event", player, index, damage);
        if (player) {
...
//           log.writeLog(log.LV_FATAL, this, "Discard Table Action Card Event", card_id);
    ...
    public function scEntrantPointUpdateEvent(player:Boolean, list:String, on:int, point:int):void {
        ...
        if (player) {
            a.forEach(function (item:*, index:int, array:Array):void {
                ActionCard.ID(item).activate(on_cards[index])
            });
...
//                commandPush(_duel.foeEntrant.updateWeaponBonus, [arraySeparateNum(8, string2intArray(foeBonus)) ,arraySeparateNum(8, string2intArray(plBonus))]);
    ...
    public function scEntrantUpdateCardValueEvent(player:Boolean, id:int, u_value:int, b_value:int, reset:Boolean):void {
        if (player) {
            ...
            commandPush(_duel.foeUpdateCardValueEvent, [id, u_value, b_value, reset]);
        }
    }
...
//                log.writeLog(log.LV_FATAL, this, dirs);
    ...
    public function scErrorNo(e:int):void {
        ...
        // エラーが開始前終了なら、終了処理を追加する
        if (e == Const.ERROR_GAME_NOT_START) {
            _ctrl.notStartErrorAbort();
        }
        // エラーが開始前に相手がいないなら、部屋を削除させる
        if (e == Const.ERROR_GAME_OPPONENT_LOGOUT) {
            if (_match != null) {
                MatchCtrl.instance.deleteRoom(_match.currentRoomId);
            }
        }
