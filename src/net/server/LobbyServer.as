...
//                player.state = Player.STATE_REGIST;
    ...
    public function scCharaCardInventoryInfo(inv_id:String, card_id:String):void {
        ...
        invID.forEach(function (item:*, index:int, array:Array):void {
            CharaCardDeck.binder.addCharaCardInventory(new CharaCardInventory(item, CharaCard.ID(cardID[index]), 0, 0))
        });
    ...
    public function scExchangeResult(success:Boolean, cardId:int, invId:int, lostInvId:String):void {
        ...
        if (success) {
            ...
            lostInvIds.forEach(function (item:*, index:int, array:Array):void {
                CharaCardDeck.binder.removeCharaCardInventoryId(item);
            });
            lostInvIds.forEach(function (item:*, index:int, array:Array):void {
                CharaCardInventory.deleteCharaCardInventory(item);
            });
...
//LOCALE_JP <-フォント読み込みを防ぐ 修正が必要
    ...
    public function scCombineResult(success:Boolean, cardId:int, invId:int):void {
        ...
        if (success) {
            ...
            lostInvIds.forEach(function (item:*, index:int, array:Array):void {
                WeaponCardDeck.binder.removeWeaponCardInventoryId(item);
            });
            lostInvIds.forEach(function (item:*, index:int, array:Array):void {
                WeaponCardInventory.deleteWeaponCardInventory(item);
            });
