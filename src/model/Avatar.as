...
//         public static const EVENT_CARD:int = 2;        // イベントカード
    ...
    public function cardsInitialize(deckNum:int, deckName:Array, deckKind:Array, deckLevel:Array, deckExp:Array, deckStutus:Array, deckCost:Array, deckMaxCost:Array, cardNum:int, invIDSet:Array, cardSet:Array, deckIndexSet:Array, positionSet:Array):void {
        ...
        // 名前つきデッキを作成
        deckName.forEach(function (item:*, index:int, array:Array):void {
            new CharaCardDeck(item, deckKind[index], deckLevel[index], deckExp[index], deckStutus[index], deckCost[index], deckMaxCost[index]);
            new WeaponCardDeck(item);
            new EquipCardDeck(item), new EventCardDeck(item)
        });

        // カードインベントリを作成
        invIDSet.forEach(function (item:*, index:int, array:Array):void {
            new CharaCardInventory(item, CharaCard.ID(cardSet[index]), deckIndexSet[index], positionSet[index])
        });
        // カードインベントリをpositionでソート
        CharaCardInventory.sortPosition();
        // デッキにカードインベントリをセット
        CharaCardInventory.cards.forEach(function (item:*, index:int, array:Array):void {
            CharaCardDeck.initCharaCardInventory(item)
        });
    }
    ...
    public function partsInitialize(partsNum:int, invIDSet:Array, partsSet:Array, usedSet:Array, endAtSet:Array):void {
        ...
        invIDSet.forEach(function (item:*, index:int, array:Array):void {
            new AvatarPartInventory(item, AvatarPart.ID(partsSet[index]), usedSet[index], endAtSet[index])
        });
    }
    ...
    public function itemsInitialize(itemNum:int, invIDSet:Array, itemsSet:Array, itemStateArray:Array):void {
        ...
        invIDSet.forEach(function (item:*, index:int, array:Array):void {
            ...
            {
                if (avatarItem.type == AvatarItem.ITEM_RAID) {
                    isAddItem = false;
                }
            }
            if (isAddItem) {
    ...
    public function slotsInitialize(slotNum:int, slotInvId:Array, slotArray:Array, slotType:Array, slotCombined:Array, slotDeckIndex:Array, slotDeckPosition:Array, slotCardPosition:Array, slotCombineData:Array):void {
        ...
        WeaponCardInventory.cards.forEach(function (item:WeaponCardInventory, index:int, array:Array):void {
            WeaponCardDeck.initWeaponCardInventory(item)
        });
        EquipCardInventory.cards.forEach(function (item:EquipCardInventory, index:int, array:Array):void {
            EquipCardDeck.initEquipCardInventory(item)
        });
        EventCardInventory.cards.forEach(function (item:EventCardInventory, index:int, array:Array):void {
            EventCardDeck.initEventCardInventory(item)
        });
...
//             return expcount;
        ...
        var ret:int = 0;
        if (_level < Const.LEVEL_MAX) {
            ret = Const.LEVEL_EXP_TABLE[_level];
        }
        return ret;
    }
