    ...
    public static function init(type:int):void {
        ...
        switch (type) {
            case TYPE_CHARA:
                ...
                cards.forEach(function (cci:CharaCardInventory, index:int, array:Array):void {
                    new InventorySet(cci)
                });
            ...
            case TYPE_WEAPON:
                ...
                WeaponCardInventory.cards.forEach(function (cci:WeaponCardInventory, index:int, array:Array):void {
                    new InventorySet(cci)
                });
                break;
            case TYPE_EQUIP:
                EquipCardInventory.sortEquipCardId();
                EquipCardInventory.cards.forEach(function (cci:EquipCardInventory, index:int, array:Array):void {
                    new InventorySet(cci)
                });
                break;
            case TYPE_EVENT:
                EventCardInventory.sortEventCardId();
                EventCardInventory.cards.forEach(function (cci:EventCardInventory, index:int, array:Array):void {
                    new InventorySet(cci)
                });
                break;
            case TYPE_MONSTER:
...
//            log.writeLog(log.LV_FATAL, type, "type is ", type, __inventories[t]);
    ...
    public function get cardInventory():ICardInventory {
        if (fileLength > 0) {
            return _invList.filter((function (item:*, index:int, ary:Array):Boolean {
                return item.index == 0
            }))[0];
        } else {
            return null;
