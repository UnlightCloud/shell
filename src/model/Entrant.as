...
//                ac.rotate(dir);
    ...
    public function cardsSweep(ac:Array /* of ActionCard */):void {
        ...
        _moveTable = _moveTable.filter(function (item:*, index:int, array:Array):Boolean {
            return (ac.indexOf(item) == -1)
        });
        // 手札から捨てたものを取り除く(ここイベント送ったほうがいいかも)
        _cards = _cards.filter(function (item:*, index:int, array:Array):Boolean {
            return (ac.indexOf(item) == -1)
        });
    ...
    public function get liveCharaCardIndex():int {
        return _hitPoints.indexOf(_hitPoints.filter(function (item:int, index:int, array:Array):Boolean {
            return item > 0
        })[0]);
    }
