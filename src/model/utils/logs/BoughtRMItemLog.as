 ...
 * チャットやゲームログを保存する
 */
public class BoughtRMItemLog extends ClientLog {
    CONFIG::LOCALE_JP
    private static const _BOUGHT_RMITEM_STR:String = "__DATA__:__NAME____RARE____LV__を__RM__で購入しました。";

    CONFIG::LOCALE_EN
    private static const _BOUGHT_RMITEM_STR:String = "__DATA__:Purchased __NAME____RARE__ __LV__ with __RM__ Credits.";

    CONFIG::LOCALE_TCN
    private static const _BOUGHT_RMITEM_STR:String = "__DATA__:您用Credit購買了__NAME____RARE____LV__和__RM__。";

    CONFIG::LOCALE_SCN
    private static const _BOUGHT_RMITEM_STR:String = "__DATA__:使用__RM__Credit购买了__NAME____RARE____LV__。";

    CONFIG::LOCALE_KR
    private static const _BOUGHT_RMITEM_STR:String = "__DATA__:__NAME____RARE____LV__を__RM__で購入しました。";

    CONFIG::LOCALE_FR
    private static const _BOUGHT_RMITEM_STR:String = "__DATA__:Vous avez acheté __NAME__ __RARE____LV__ avec vos __RM__ crédits.";

    CONFIG::LOCALE_ID
    private static const _BOUGHT_RMITEM_STR:String = "__DATA__:__NAME____RARE____LV__を__RM__で購入しました。";

    CONFIG::LOCALE_TH
    private static const _BOUGHT_RMITEM_STR:String = "__DATA__:ทำการซื้อ __NAME____RARE____LV__ ด้วย __RM__ แล้ว.";


    public function BoughtRMItemLog() {
        super();
    }

    public override function get type():int {
        return ClientLog.BOUGHT_RMITEM;
    }
    ...
    private function setText(i:int):String {
        ...
        if (date != null && nameData != null) {
            ...
            if (nameData[0] == Const.TG_CHARA_CARD) {
                ret = _BOUGHT_RMITEM_STR.replace(/__DATA__/, TimeFormat.transDateStr(date)).replace(/__NAME__/, nameData[2]);
                if (nameData[1] < 1000) {
                ...
                ret = _BOUGHT_RMITEM_STR.replace(/__DATA__/, TimeFormat.transDateStr(date)).replace(/__NAME__/, nameData[2] + "GEM");
                ret = ret.replace(/__RARE__/, "").replace(/__LV__/, "");
            }// その他の場合
            else {
                ret = _BOUGHT_RMITEM_STR.replace(/__DATA__/, TimeFormat.transDateStr(date)).replace(/__NAME__/, nameData[2]);
