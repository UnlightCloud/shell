...
//             // フラグを見てカードを保存する
        ...
        // 変更のあるカードのみ変更
        CharaCardInventory.getDirtyFlagCardList().forEach(function (item:*, index:int, array:Array):void {
            dispatchEvent(new EditCardEvent(EditCardEvent.UPDATE_CHARA_CARD, item));
        });
        WeaponCardInventory.cards.forEach(function (item:*, index:int, array:Array):void {
            if (item.save()) {
                dispatchEvent(new EditCardEvent(EditCardEvent.UPDATE_WEAPON_CARD, item))
            }
        });
        EventCardInventory.cards.forEach(function (item:*, index:int, array:Array):void {
            if (item.save()) {
                dispatchEvent(new EditCardEvent(EditCardEvent.UPDATE_EVENT_CARD, item))
            }
        });
