...
//            reload();
    ...
    public function showInventory(dc:BaseDeckClip, index:int):void {
        ...
        pExec.addThread(new BeTweenAS3Thread(dc, {
            x: _DECK_X + _DECK_OFFSET_X * (index - 1) + _DECK_OFFSET_X2 * ((index + 1) % 2),
            y: _DECK_Y + _DECK_OFFSET_Y * ((index + 1) % 2),
            rotationY: _DECK_ROTATE[index],
            scaleX: _DECK_SCALE_MAX - _DECK_SCALE_OFFSET * ((index + 1) % 2),
            scaleY: _DECK_SCALE_MAX - _DECK_SCALE_OFFSET * ((index + 1) % 2),
            alpha: 1.0
