...
//             _ccc = new CharaCardClip(cc);
        ...
        _textSet.forEach(function (item:*, index:int, array:Array):void {
            initState(item, index)
        });

        if (checkTypeChara(true))
//            if (_type != InventorySet.TYPE_CHARA)
        {
            _status.forEach(function (item:*, index:int, array:Array):void {
                if (index == 1) {
                    initColorString(item)
                }
            });
        }
