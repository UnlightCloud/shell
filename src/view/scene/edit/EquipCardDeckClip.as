 ...
 *
 */
public class EquipCardDeckClip extends BaseDeckClip {
    // 翻訳データ
    CONFIG::LOCALE_JP
...
//                ["所持しているデッキです。\nデッキに存在するカードが表示されます。"],
     ...
     *
     */
    public function EquipCardDeckClip(deck:IDeck) {
        super(deck);
        _deckCase.alpha = 0.5;
    }
    ...
    protected override function clipInitialize(cci:ICardInventory):void {
        log.writeLog(log.LV_FATAL, this, "clip initialize equip!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        if (_cccDic[cci] == null) {
            var cc:EquipCardClip = new EquipCardClip(EquipCard(cci.card));
            cc.cardInventory = cci;
            _cccDic[cci] = cc;
//                Unlight.GCW.watch(cc);

        }
        _cccDic[cci].x = _CARD_X + _CARD_OFFSET_X * EquipCardInventory(_cccDic[cci].cardInventory).position;
...
//            EquipCardClip(cc).getEditHideThread().start();
        EquipCardClip(cc).getHideThread().start();
    }
...
//            new TweenerThread(cc, {x:_CARD_X + _CARD_OFFSET_X * cc.charaCardInventory.position, y:_CARD_Y, transition:"easeOutSine", time:0.20}).start();
        new BeTweenAS3Thread(cc, {
            x: _CARD_X + _CARD_OFFSET_X * EquipCardInventory(cc.cardInventory).position,
            y: _CARD_Y
        }, null, 0.20, BeTweenAS3Thread.EASE_OUT_SINE).start();
    }

    // カードを消す
    protected override function removeCard(cc:ICardClip):void {
        _deckEditor.deckToBinderEquipCard(EquipCardInventory(cc.cardInventory));
    }


    // カードが追加される
    public override function binderToDeckHandler(e:EditCardEvent):void {
        if (EquipCardDeck.decks[e.cci.index] == _deck) {
            addCard(e.cci);
        }
    }


    public function get charaCardDeck():EquipCardDeck {
        return EquipCardDeck(_deck);
    }
}
}
