 ...
 *
 */
public class CombineSetArea extends BaseScene {
    // 翻訳データ
    CONFIG::LOCALE_JP
...
//        private var _deck:CharaCardDeck;                              // デッキのポインタ
    ...
    // 定数
    private static const _DECK_X:int = 0;                      // デッキコンテナX位置
    private static const _DECK_Y:int = 430;                      // デッキコンテナY位置
    // private static const _DECK_OFFSET_X:int = 0;                   // デッキXずれ
    // private static const _DECK_OFFSET_X2:int =0;                // デッキXずれ
    // private static const _DECK_OFFSET_Y:int = 0;                   // デッキYずれ
    // private static const _DECK_SCALE_MAX:Number = 1.0;          // デッキコンテナ最大サイズ
    // private static const _DECK_SCALE_OFFSET:Number = 1.0;          // デッキコンテナ最小サイズ
    ...
    private static const _DECK_SCALE_MAX:Number = 1.0;          // デッキコンテナ最大サイズ
    private static const _DECK_SCALE_OFFSET:Number = 0.5;          // デッキコンテナ最小サイズ
    private static const _DECK_ROTATE:Array = [-80, 0, 70];             // デッキ回転角
...
//                 "デッキの番号です。",],
    ...
    public static function getCharaCardDeck():CombineSetArea {
        log.writeLog(log.LV_INFO, "chara card deck");
        return new CombineSetArea(InventorySet.TYPE_CHARA);
    }

    public static function getWeaponCardDeck():CombineSetArea {
        log.writeLog(log.LV_INFO, "weapon card deck");
        return new CombineSetArea(InventorySet.TYPE_WEAPON);
    }

    public static function getEquipCardDeck():CombineSetArea {
        log.writeLog(log.LV_INFO, "equip card deck");
        return new CombineSetArea(InventorySet.TYPE_EQUIP);
    }

    public static function getEventCardDeck():CombineSetArea {
        log.writeLog(log.LV_INFO, "equip card deck");
        return new CombineSetArea(InventorySet.TYPE_EVENT);
    }

    public static function getMonsterCardDeck():CombineSetArea {
        log.writeLog(log.LV_INFO, "monster card deck");
        return new CombineSetArea(InventorySet.TYPE_MONSTER);
    }

    public static function getOtherCardDeck():CombineSetArea {
        log.writeLog(log.LV_INFO, "other card deck");
        return new CombineSetArea(InventorySet.TYPE_OTHER);
    }
     ...
     *
     */
    public function CombineSetArea(type:int) {
        _type = type;
...
//            Unlight.GCW.watch(_timer);
    ...
    private function initProperty():void {
        ...
        _cost.styleName = "DeckCostLabelForCombineSetArea";
    ...
    private function setCCD(ccd:IDeck, type:String):void {
        ...
        log.writeLog(log.LV_FATAL, this, "set ccc", _ccdDic[ccd]);
        if (_ccdDic[ccd] == null) {
            // ここはタイプで分ける
            switch (_type) {
...
//                    _ccdDic[ccd] = new CharaCardDeckClip(ccd);
                    _ccdDic[ccd] = new CombineSetClip(ccd);
                    break;
...
//            reload();
    ...
    private function loadDeckName():void {
        ...
        if (currentIndex == _deckEditor.currentIndex) {
            _no.styleName = "CurrentDeckNameLabelForCombineSetArea";
            _name.styleName = "CurrentDeckNameLabelForCombineSetArea";
        } else {
            _no.styleName = "DeckNameLabelForCombineSetArea";
            _name.styleName = "DeckNameLabelForCombineSetArea";
        }
    ...
    private function loadDeckData(kind:int = 0):void {
        ...
        // コスト更新
        _cost.styleName = costCalc > costMax ? "DeckCostLabelRedForCombineSetArea" : "DeckCostLabelForCombineSetArea";
    ...
    public function show():void {
        showName();
        showData();
        _deckClips.forEach(function (dc:BaseDeckClip, index:int, array:Array):void {
            showInventory(dc, index)
        });
    }
    ...
    public function showInventory(dc:BaseDeckClip, index:int):void {
        ...
        pExec.addThread(new BeTweenAS3Thread(dc, {
            x: 0,
            y: 0,
            rotationY: 0,
            scaleX: 1.0,
            scaleY: 1.0,
            alpha: 1.0
        }, null, 0.15, BeTweenAS3Thread.EASE_IN_SINE));
...
//            updateEventHandler();
    ...
    // 候補リストをリセットする
    public function resetSetList():void {
        for (var key:Object in _ccdDic) {
            _ccdDic[key].resetData();
        }
    }
...
class ShowThread extends BaseShowThread {
    public function ShowThread(da:CombineSetArea, stage:DisplayObjectContainer, at:int) {
        super(da, stage);
    }
...
class HideThread extends BaseHideThread {
    public function HideThread(da:CombineSetArea) {
        super(da);
    }
...
class SetFlagThread extends Thread {
    private var _da:CombineSetArea;
    private var _flag:Boolean;

    public function SetFlagThread(da:CombineSetArea, flag:Boolean) {
        _da = da;
        _flag = flag;
    }
