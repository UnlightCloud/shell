...
//            log.writeLog(log.LV_FATAL, this, "deck clip init",_deck);
        if (_deck) {
            _deck.cardInventories.forEach(function (cci:ICardInventory, index:int, array:Array):void {
                clipInitialize(cci)
            });
        }
...
//            new TweenerThread(cc, {x:_CARD_X + _CARD_OFFSET_X * cc.charaCardInventory.position, y:_CARD_Y, transition:"easeOutSine", time:0.20}).start();
        new BeTweenAS3Thread(cc, {
            x: _CARD_X + _CARD_OFFSET_X * CharaCardInventory(cc.cardInventory).position,
            y: _CARD_Y
