...
/**
 * エディット画面のバインダー部分のビュークラス
 *
 */
public class BinderArea extends BaseScene {
    ...
    private static const _COMBINE_X:int = 0;            // デッキエリアの開始X
    private static const _COMBINE_Y:int = 445;          // デッキエリアの開始Y
    private static const _COMBINE_X2:int = 560;          // デッキエリアの終端X
    private static const _COMBINE_Y2:int = 665;         // デッキエリアの終端Y
...
//         private var _gcTestCC:CharaCardClip;
    ...
    public static function getCharaCardBinder():BinderArea {
        log.writeLog(log.LV_INFO, "chara card binder");
        return new BinderArea(InventorySet.TYPE_CHARA);
    }

    public static function getWeaponCardBinder():BinderArea {
        log.writeLog(log.LV_INFO, "weapon card binder");
        return new BinderArea(InventorySet.TYPE_WEAPON);
    }

    public static function getEquipCardBinder():BinderArea {
        log.writeLog(log.LV_INFO, "equip card binder");
        return new BinderArea(InventorySet.TYPE_EQUIP);
    }

    public static function getEventCardBinder():BinderArea {
        log.writeLog(log.LV_INFO, "equip card binder");
        return new BinderArea(InventorySet.TYPE_EVENT);
    }

    public static function getMonsterCardBinder():BinderArea {
        log.writeLog(log.LV_INFO, "monster card binder");
        return new BinderArea(InventorySet.TYPE_MONSTER);
    }

    public static function getOtherCardBinder():BinderArea {
        log.writeLog(log.LV_INFO, "other card binder");
        return new BinderArea(InventorySet.TYPE_OTHER);
    }
     ...
     *
     */
    public function BinderArea(type:int) {
        _type = type;
...
//            _pageNo.text = "1";
    ...
    public override function init():void {
        ...
        _combine.addEventListener(CombineEvent.COMBINE_SUCCESS, exchangeSuccessHandler);

        _deckEditor.addEventListener(EditDeckEvent.BINDER_SELECT, binderSelectHandler);
    ...
    public override function final():void {
        ...
        _combine.removeEventListener(CombineEvent.COMBINE_SUCCESS, exchangeSuccessHandler);

        _deckEditor.removeEventListener(EditDeckEvent.BINDER_SELECT, binderSelectHandler);
    ...
    public function loadInventory(invSet:InventorySet, index:int):void {
        // ICard
        var cc:ICard = invSet.card;
//            log.writeLog(log.LV_DEBUG, this, "load inventory", cc, _cardContainers.length, index );
        ...
        } else if (_cccDic[cc] == null) {
            log.writeLog(log.LV_FATAL, this, "load inventory renew card", index);
            setCCC(cc, index);
        }
    ...
    private function setCCC(cc:ICard, index:int):void {
        log.writeLog(log.LV_DEBUG, this, "set ccc type", _type);

//            log.writeLog(log.LV_FATAL, this, "set ccc",_cccDic[cc], cc.id,_cccDicCacheUpdate.length, _CCC_CACHE_MAX);
        ...
        var wcForceReload:Boolean = false;
        if (_type == InventorySet.TYPE_WEAPON) {
            wcForceReload = WeaponCard(cc).forceReload;
        }
        log.writeLog(log.LV_DEBUG, this, "set ccc type!!!!!!!!", cc.id, _type, forceReload, _cccDic[cc]);
        if (_cccDic[cc] == null || wcForceReload) {
//                Unlight.GCWOn();
...
//                     Unlight.GCW.watch(_cccDic[cc] );
        ...
        if (wcForceReload) {
            WeaponCard(cc).forceReload = false;
        }
...
//            log.writeLog(log.LV_FATAL, this, "mouse down ");
    ...
    private function mouseUpHandler(e:MouseEvent):void {
        ...
        if (collision(e)) {
            log.writeLog(log.LV_INFO, this, "mouseUpHandler", "true");
            removeCard(UIComponent(e.currentTarget), index);
        } else {
            log.writeLog(log.LV_INFO, this, "mouseUpHandler", "false");
            new BeTweenAS3Thread(e.currentTarget, {
                x: _INV_X + _OFFSET_X * (index % _ROW),
                y: _INV_Y + _OFFSET_Y * int(index / _COLUMN) - int(_PAGE_SIZE / 2 - 1 < index) * _OFFSET_Y * _COLUMN
            }, null, 0.2, BeTweenAS3Thread.EASE_OUT_SINE).start();
        }
    }

    // バインダーとの当たり判定を返す関数
    private function collision(e:MouseEvent):Boolean {
//            log.writeLog(log.LV_INFO, this, "e.x is", e.stageX, "e.y is", e.stageY);
...
//                log.writeLog(log.LV_INFO, this, "collision!!!!!");
            ...
            log.writeLog(log.LV_DEBUG, this, "COMBINE COLLISION", "e.x is", e.stageX, "e.y is", e.stageY);
            if (e.stageX > _COMBINE_X && e.stageX < _COMBINE_X2 && e.stageY > _COMBINE_Y && e.stageY < _COMBINE_Y2) {
                log.writeLog(log.LV_DEBUG, this, "COMBINE COLLISION", "true");
                return true;
            } else {
                return false;
            }
    ...
    private function addCard(cci:ICardInventory):void {
        _currentPageInvSet.forEach(function (item:*, index:int, array:Array):void {
            if (item.id == cci.card.id) {
                loadInventory(item, index)
            }
        });
    }
    ...
    private function removeCard(uic:UIComponent, index:int):void {
        ...
        switch (_type) {
            ...
            case InventorySet.TYPE_OTHER:
                ...
                break;
            case InventorySet.TYPE_WEAPON:
                _deckEditor.binderToDeckWeaponCard(WeaponCardClip(uic.getChildAt(0)).cardInventory, _deckEditor.selectIndex, _currentCharaCardPos);
                log.writeLog(log.LV_INFO, this, "set weapon card");
                break;
//                 case InventorySet.TYPE_EQUIP:
//                     _cccDic[cc] = new EquipCardClip(EquipCard(cc));
...
//            reload();
    ...
    public function binderSelectHandler(e:EditDeckEvent):void {
        InventorySet.init(_type);
        _invSet = InventorySet.inventories;
    ...
    // カードが合成される
    public function exchangeSuccessHandler(e:Event):void {
        update();
        forceReload = true;
    }
...
class ShowThread extends BaseShowThread {
    public function ShowThread(ba:BinderArea, stage:DisplayObjectContainer, at:int) {
        super(ba, stage);
    }
...
class HideThread extends BaseHideThread {
    public function HideThread(ba:BinderArea) {
        super(ba);
    }
...
class PageReloadThread extends Thread {
    private var _ba:BinderArea;
    private var _oldPage:int = 0;

    public function PageReloadThread(ba:BinderArea) {
        _ba = ba;
        super();
    }
