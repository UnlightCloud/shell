    ...
    private function updateList():void {
        ...
        for (var key:Object in f_set) {
            if (convertTabType(f_set[key].achievement.kind) != -1 && f_set[key].achievement.checkTimeOver()) {
                ...
                if (_achievementSceneDic[f_set[key].achievement.id] != null) {
                    _achievementSceneDic[f_set[key].achievement.id].visible = false
                }
            }
        }
        ...
        allAchievementSet.forEach(function (item:*, index:int, array:Array):void {
            _achievementInvSet[TAB_ALL].push(item)
        });
        allFinAchievementSet.forEach(function (item:*, index:int, array:Array):void {
            _finAchievementInvSet[TAB_ALL].push(item)
        });
    }
