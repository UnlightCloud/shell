...
//             _maskBitmapData = new BitmapData(width, textHeight,true, 0x00000000);
    ...
    public function getReadEndPosition():Point {
        ...
        if (r == null) {
            r = getCharBoundaries(_currentPotision - 2)
        }
    ...
    public function getScrollLineThread(line:int):Thread {
        ...
        return new TweenerThread(_maskBitmap, {
            _scrollRect_y: _infoLines[line][0],
            transition: "easeOutCirc",
            time: 0.0005 * lineNum + 0.0002
        })
    }
...
class TransitionThread extends Thread {
    ...
    private function burning():void {
        ...
        if (checkInterrupted()) {
            ...
            if (beginRect == null) {
                beginRect = _tf.getCharBoundaries(_begin + 1)
            }
            endRect = _tf.getCharBoundaries(_end);
            ;
            if (endRect == null) {
                endRect = _tf.getCharBoundaries(_end - 1)
            }
