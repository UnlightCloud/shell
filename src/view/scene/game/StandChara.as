...
//             _plFilter = null;
    ...
    public function getBringOffThread(result:int = 0):Thread {
        ...
        if (result) {
            if (_foeChara != null) {
                pExec.addThread(new BeTweenAS3Thread(_foeChara, {x: 700}, null, 1.0, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
            }
            if (_plChara != null) {
                pExec.addThread(new BeTweenAS3Thread(_plChara, {alpha: 0}, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
            }
            sExec.addThread(pExec);
        } else {
            if (_plChara != null) {
                pExec.addThread(new BeTweenAS3Thread(_plChara, {x: -700}, null, 1.0, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
            }
            if (_foeChara != null) {
                pExec.addThread(new BeTweenAS3Thread(_foeChara, {alpha: 0}, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
            }
            sExec.addThread(pExec);
        }
