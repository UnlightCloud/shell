...
//            __instance =null;
    ...
    public function getBringOffThread():Thread {
        var pExec:ParallelExecutor = new ParallelExecutor();
        if (_playerAvatarClip != null) {
            pExec.addThread(new BeTweenAS3Thread(_playerAvatarClip, {alpha: 0.0}, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
        }
        if (_playerAvatarBase != null) {
            pExec.addThread(new BeTweenAS3Thread(_playerAvatarBase, {alpha: 0.0}, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
        }
        return pExec;
    }
