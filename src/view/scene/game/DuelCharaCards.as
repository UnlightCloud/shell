...
//            log.writeLog(log.LV_FATAL, this, "DuelCards final end");
    ...
    private function bossBuffHandler(e:BuffEvent):void {
        if (_state == _STATE_END && _duel.raidBoss) {
            return
        }
        var currentIndex:int = _duel.foeEntrant.currentCharaCardIndex;
...
//            log.writeLog(log.LV_FATAL, this, "change start handler",_duel.plEntrant.getChangeDone() );
                ...
                for (var i:int = 0; i < _playerCharaCardClips.length; i++) {
                    ...
                    if (i == currentIndex) {
                        if (_duel.plEntrant.hitPoint) {
                            pExec.addThread(new BeTweenAS3Thread(_playerCharaCardClips[i], {
                                x: _PLAYER_CHARA_CARD_X[0],
                                y: _PLAYER_CHARA_CARD_Y[0],
                                scaleX: _CHARA_CARD_SCALE[0],
                                scaleY: _CHARA_CARD_SCALE[0],
                                alpha: 1
                            }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE));
                        } else {
                            pExec.addThread(new BeTweenAS3Thread(_playerCharaCardClips[i], {
                                x: _PLAYER_CHARA_CARD_X[0],
                                y: _PLAYER_CHARA_CARD_Y[0],
    ...
    private function plCharaCardChangeHandler(e:Event):void {
        ...
        for (var i:int = 0; i < _playerCharaCardClips.length; i++) {
            if (i > currentIndex) {
                pExec.addThread(new BeTweenAS3Thread(_playerCharaCardClips[i], {
                    x: _PLAYER_CHARA_CARD_X[i],
                    y: _PLAYER_CHARA_CARD_Y[i],
                    scaleX: _CHARA_CARD_SCALE[i],
                    scaleY: _CHARA_CARD_SCALE[i],
                    alpha: _CHARA_CARD_ALPHA
                }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE));
                ...
                pExec.addThread(new BeTweenAS3Thread(_playerCharaCardClips[i], {
                    x: _PLAYER_CHARA_CARD_X[i + 1],
                    y: _PLAYER_CHARA_CARD_Y[i + 1],
                    scaleX: _CHARA_CARD_SCALE[i + 1],
                    scaleY: _CHARA_CARD_SCALE[i + 1],
                    alpha: _CHARA_CARD_ALPHA
                }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE));
...
//                 _playerCharaCardClips[i].removeEventListener(MouseEvent.CLICK, clickCharaChangeHandler);
    ...
    private function foeCharaCardChangeHandler(e:Event):void {
        ...
        for (var i:int = i; i < _foeCharaCardClips.length; i++) {
            if (i > currentIndex) {
                pExec.addThread(new BeTweenAS3Thread(_foeCharaCardClips[i], {
                    x: _FOE_CHARA_CARD_X[i],
                    y: _FOE_CHARA_CARD_Y[i],
                    scaleX: _CHARA_CARD_SCALE[i],
                    scaleY: _CHARA_CARD_SCALE[i]
                }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE));
                ...
                pExec.addThread(new BeTweenAS3Thread(_foeCharaCardClips[i], {
                    x: _FOE_CHARA_CARD_X[i + 1],
                    y: _FOE_CHARA_CARD_Y[i + 1],
                    scaleX: _CHARA_CARD_SCALE[i + 1],
                    scaleY: _CHARA_CARD_SCALE[i + 1]
                }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE));
                ...
                pExec.addThread(new BeTweenAS3Thread(_foeCharaCardClips[i], {
                    x: _FOE_CHARA_CARD_X[0],
                    y: _FOE_CHARA_CARD_Y[0],
                    scaleX: _CHARA_CARD_SCALE[0],
                    scaleY: _CHARA_CARD_SCALE[0]
                }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE));
...
//                 BetweenAS3.tween(ccc, {x:_ENLARGE_PLAYER_CHARA_CARD_X, y:_ENLARGE_PLAYER_CHARA_CARD_Y, scaleX:_ENLARGE_CHARA_CARD_SCALE, scaleY:_ENLARGE_CHARA_CARD_SCALE}, null, 0.15, Quad.easeOut).play();
            BetweenAS3.tween(ccc, {
                y: _ENLARGE_PLAYER_CHARA_CARD_Y,
                scaleX: _ENLARGE_CHARA_CARD_SCALE,
                scaleY: _ENLARGE_CHARA_CARD_SCALE,
                alpha: 1.0
            }, null, 0.15, Quad.easeOut).play();
...
//                var resetTween:ITween= BetweenAS3.tween(_enlargedCard, {x:_enlargeBeforeX, y:_enlargeBeforeY, scaleX:_CHARA_CARD_SCALE[1], scaleY:_CHARA_CARD_SCALE[1]}, null, 0.08, Quad.easeOut);
            ...
            var resetTween:ITween = BetweenAS3.tween(_enlargedCard, {
                x: _enlargedCardsPoints[_enlargedCard][0],
                y: _enlargedCardsPoints[_enlargedCard][1],
                scaleX: _CHARA_CARD_SCALE[1],
                scaleY: _CHARA_CARD_SCALE[1],
                alpha: _CHARA_CARD_ALPHA
            }, null, 0.08, Quad.easeOut);
...
//                setChildIndex(ccc, numChildren-1 );
    ...
    public function getBringOnAfterThread():Thread {
        ...
        var plTween:Thread = new BeTweenAS3Thread(_playerCharaCardClips[0], {
            x: _PLAYER_CHARA_CARD_X[0],
            y: _PLAYER_CHARA_CARD_Y[0],
            scaleX: 1.0,
            scaleY: 1.0
        }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE);
        var foeTween:Thread = new BeTweenAS3Thread(_foeCharaCardClips[0], {
            x: _FOE_CHARA_CARD_X[0],
            y: _FOE_CHARA_CARD_Y[0],
            scaleX: 1.0,
            scaleY: 1.0
