...
//                log.writeLog(log.LV_INFO, this, "push item!!!!",_itemList.parent, _itemList.visible, _itemList.x, _itemList.alpha, _resultStep.parent);
            new BeTweenAS3Thread(_itemList, {
                x: -250,
                alpha: 0.0
            }, null, 0.3 / _RESULT_SCENE_SPEED, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false).start();
        } else {
//                log.writeLog(log.LV_INFO, this, "push item!!!!",_itemList.parent, _itemList.visible, _itemList.x, _itemList.alpha, _resultStep.parent);
            new BeTweenAS3Thread(_itemList, {
                x: 25,
                alpha: 1.0
            }, null, 0.3 / _RESULT_SCENE_SPEED, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, true).start();
        }
    }
...
//            log.writeLog(log.LV_FATAL, this, "object cards", rewardObject, label, thread);
    ...
    public function getResultThread(result:int, gems:int, exp:int, expBonus:int, gemsPow:int, expPow:int, totalGems:int, totalExp:int):Thread {
        ...
        sExec.addThread(new ClousureThread(function ():void {
            _resultButton.buttonEnabled = true
        }));
...
//            _resultStep.seRank(_reward.bonus);
        ...
        sExec.addThread(new BeTweenAS3Thread(_itemList, {
            x: -250,
            alpha: 0.0
        }, null, 0.3 / _RESULT_SCENE_SPEED, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
...
//            _stepLabel.text = "STEP "+ _reward.bonus.toString();
        ...
        if (_ccc) {
            ...
            // スレッドを合成
            pExec.addThread(new BeTweenAS3Thread(_prevCards[_prevCards.length - 1], {
                x: _PREV_CARD_X,
                y: _PREV_CARD_Y,
                scaleX: _PREV_CARD_SCALE,
                scaleY: _PREV_CARD_SCALE
            }, null, 1.0 / _RESULT_SCENE_SPEED, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, true));
            // 枚数ラベルを追加
            _prevCardsNum.push(_cccNum);
            // スレッドを合成
            pExec.addThread(new BeTweenAS3Thread(_prevCardsNum[_prevCardsNum.length - 1], {
                x: _PREV_CARD_NUM_X,
                y: _PREV_CARD_NUM_Y
            }, null, 1.0 / _RESULT_SCENE_SPEED, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, true));
        ...
        pExec.addThread(new BeTweenAS3Thread(_ccc, {
            x: _CCC_X,
            y: _CCC_Y,
            scaleX: 1.0,
            scaleY: 1.0
        ...
        pExec.addThread(new BeTweenAS3Thread(_cccNum, {
            x: _CCC_NUM_X,
            y: _CCC_NUM_Y,
            scaleX: 1.0,
            scaleY: 1.0
...
//                sExec.addThread(new ShowGetThread(_cardFrame, this));
            ...
            // 作られたキャラカードクリップを消す
            _nextCardsNum.forEach(function (item:*, index:int, array:Array):void {
                pExec.addThread(new ObjectHideThread(item))
            });
            _prevCardsNum.forEach(function (item:*, index:int, array:Array):void {
                pExec.addThread(new ObjectHideThread(item))
            });
            _nextCards.forEach(function (item:*, index:int, array:Array):void {
                pExec.addThread(getHideCCCThread(item))
            });
            _prevCards.forEach(function (item:*, index:int, array:Array):void {
                pExec.addThread(getHideCCCThread(item))
            });
            // リザルトを消す
...
//                sExec.addThread(new HideGetThread(_cardFrame));
            ...
            // カードを表示
            pExec2.addThread(new BeTweenAS3Thread(_ccc, {
                x: _CCC_DIST_X,
                y: _CCC_DIST_Y
            }, null, 0.5 / _RESULT_SCENE_SPEED, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, true));
            pExec2.addThread(new BeTweenAS3Thread(_cccNum, {
                x: _CCC_DIST_NUM_X,
                y: _CCC_DIST_NUM_Y
            }, null, 0.5 / _RESULT_SCENE_SPEED, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, true));
