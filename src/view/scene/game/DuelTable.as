...
//            _phaseTimer = null;
        ...
        if (_plDiceManager != null) {
            _plDiceManager.destroy()
        }
        if (_foeDiceManager != null) {
            _foeDiceManager.destroy();
        }
        _plDiceManager = null;
...
//            GameCtrl.instance.addNoWaitViewSequence(_foeHand.getVanishHandCardsThread(e.actionCards));
    ...
    private function battlePlayerResultHandler(e:RollResultEvent):void {
        ...
        _foeDiceManager.eyeArray = e.defDice;
        y
                // 自分と相手のダイスを出す
        var pExec:ParallelExecutor = new ParallelExecutor();
        var pExec2:ParallelExecutor = new ParallelExecutor();
...
//          }
    ...
    public function getAllHideThread():Thread {
        var plThread:ParallelExecutor = new ParallelExecutor();
        if (_phaseTimer != null) {
            plThread.addThread(_phaseTimer.getHideThread());
        }
        if (_dropTable != null) {
            plThread.addThread(_dropTable.getHideThread());
        }
        if (_foeHand != null) {
            plThread.addThread(_foeHand.getHideThread());
        }
        if (_playerHand != null) {
            plThread.addThread(_playerHand.getHideThread());
        }
        return plThread;
    }
    ...
    public function getBringOffThread():Thread {
        var pExec:ParallelExecutor = new ParallelExecutor();
        if (_baseCard != null) {
            pExec.addThread(_baseCard.getBringOffThread());
        }
        if (_positionArea != null) {
            pExec.addThread(_positionArea.getBringOffThread());
        }
        if (_deckArea != null) {
            pExec.addThread(_deckArea.getBringOffThread());
        }
        if (_foeHand != null) {
            pExec.addThread(_foeHand.getBringOffThread());
        }
        if (_playerHand != null) {
            pExec.addThread(_playerHand.getBringOffThread());
        }
        if (_dropTable != null) {
            pExec.addThread(_dropTable.getBringOffThread());
        }
        if (_phaseTimer != null) {
            pExec.addThread(_phaseTimer.getBringOffThread());
        }
        var i:int;
        if (_playerCharaBuffsTable != null) {
