...
public class BaseHand extends BaseScene {
    ...
    public override function final():void {
        dropTable = null;
        acArray.forEach(function (item:ActionCardClip, index:int, array:Array):void {
            item.final();
            disposeHandlers(item)
        });
        tableArray.forEach(function (item:ActionCardClip, index:int, array:Array):void {
            item.final();
            disposeHandlers(item)
        });
...
//             sExec.addThread(new DealThread(acc, registHandlers, w, acArray.length-1, spacing*align));
    ...
    public function getAddEventACThread(acc:ActionCardClip, wait:Number):Thread {
        ...
        addChild(acc);
        var sExec:SerialExecutor = new SerialExecutor();
        sExec.addThread(new BeTweenAS3Thread(acc, {
            alpha: 1.0,
...
//             acc.removeEventListener(MouseEvent.MOUSE_OUT,mouseRemoveHandler);
    ...
    protected function updateHandMove():Thread {
        ...
        for (var i:int = 0; i < len; i++) {
            if (acSet[i] != null) {
                var sExec:SerialExecutor = new SerialExecutor();
                sExec.addThread(new BeTweenAS3Thread(acSet[i], {
                    x: getHandX(i),
                    y: getHandY(i),
                    scaleX: handCardsScale(),
                    scaleY: handCardsScale()
...
//             {
        ...
        for (var i:int = 0; i < taSet.length; i++) {
            if (taSet[i] != null) {
                ...
                _dropPExec.addThread(new BeTweenAS3Thread(taSet[i], {
                    x: getDropX(i),
                    y: getDropY(i),
