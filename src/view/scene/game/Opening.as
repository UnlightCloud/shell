...
//                 });
    ...
    private function finalizeCharaCardName():void {
        _plName.forEach(function (item:*, index:int, array:Array):void {
            _nameContainer.removeChild(item)
        });
        _plSubName.forEach(function (item:*, index:int, array:Array):void {
            _nameContainer.removeChild(item)
        });
        _foeName.forEach(function (item:*, index:int, array:Array):void {
            _nameContainer.removeChild(item)
        });
        _foeSubName.forEach(function (item:*, index:int, array:Array):void {
            _nameContainer.removeChild(item)
        });
    }
    ...
    public function getBringOffThread(opPop:Thread = null):Thread {
        ...
        pExec.addThread(new BeTweenAS3Thread(_plChara, {
            x: _PLAYER_X - 500,
            alpha: 0.5
        }, null, 0.9, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
        pExec.addThread(new BeTweenAS3Thread(_foeChara, {
            x: _FOE_X + 500,
            alpha: 0.5
        }, null, 0.9, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
        //pExec.addThread(new BeTweenAS3Thread(_opUp, {y:-150 ,alpha:0.0}, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, 0.0 ,false ));
        pExec.addThread(new BeTweenAS3Thread(_opDown, {
            y: 150,
            alpha: 0.0
        }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
        if (opPop) {
            pExec.addThread(opPop);
        }
        pExec.addThread(new BeTweenAS3Thread(_nameContainer, {
            y: 150,
            alpha: 0.0
        }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
        pExec.addThread(new BeTweenAS3Thread(_opName, {
            y: 150,
            alpha: 0.0
        }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
    ...
    public function getUpBarBringOffThread():Thread {
        var pExec:ParallelExecutor = new ParallelExecutor();
        pExec.addThread(new BeTweenAS3Thread(_opUp, {
            y: -150,
            alpha: 0.0
        }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false));
        return pExec;
