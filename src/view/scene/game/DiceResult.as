...
//         private const _PL_DICE_X:int = 300;        // 
    ...
    public function getResultThread(plDice:Array, plType:int, foeDice:Array, foeType:int):Thread {
        ...
        for (i = 0; i < _plDiceImage.length; i++) {
            pExec2.addThread(new BeTweenAS3Thread(_plDiceImage[i], {
                x: 263 + (i % 5) * 60,
                y: 380 + int(i / 5) * 60
            }, null, 0.3, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, true));
        }
        for (i = 0; i < _foeDiceImage.length; i++) {
            pExec2.addThread(new BeTweenAS3Thread(_foeDiceImage[i], {
                x: 263 + (i % 5) * 60,
                y: 280 - int(i / 5) * 60
            }, null, 0.3, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, true));
        }
        ...
        for (i = 0; i < _plDiceImage.length || i < _foeDiceImage.length; i++) {
            if (_plDiceImage[i]) {
                if (_foeDiceImage[i]) {
                    ...
                    pExec3.addThread(new BeTweenAS3Thread(_plDiceImage[i], {
                        scaleX: 1.2,
                        scaleY: 1.2,
                        alpha: 0.0
                    }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, i * 0.1, false));
                } else {
                    if (plType == 0) {
            ...
            if (_foeDiceImage[i]) {
                if (_plDiceImage[i]) {
                    ...
                    pExec3.addThread(new BeTweenAS3Thread(_foeDiceImage[i], {
                        scaleX: 1.2,
                        scaleY: 1.2,
                        alpha: 0.0
                    }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, i * 0.1, false));
...
//            _plDiceImage.forEach(function(item:*, index:int, array:Array):void{pExec4.addThread(new TweenerThread(item, {alpha:0.0, transition:"easeOutSine", time:0.3, hide:true}))});
        _plDiceImage.forEach(function (item:*, index:int, array:Array):void {
            pExec4.addThread(new BeTweenAS3Thread(item, {alpha: 0.0}, null, 0.3, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false))
        });
//            _foeDiceImage.forEach(function(item:*, index:int, array:Array):void{pExec4.addThread(new TweenerThread(item, {alpha:0.0, transition:"easeOutSine", time:0.3, hide:true}))});
        _foeDiceImage.forEach(function (item:*, index:int, array:Array):void {
            pExec4.addThread(new BeTweenAS3Thread(item, {alpha: 0.0}, null, 0.3, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, false))
        });
        ...
        // ダイスを消去
        _plDiceImage.forEach(function (item:*, index:int, array:Array):void {
            pExec5.addThread(item.getHideThread())
        });
        _foeDiceImage.forEach(function (item:*, index:int, array:Array):void {
            pExec5.addThread(item.getHideThread())
        });
