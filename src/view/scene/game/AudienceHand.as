    ...
    protected function updateOpenDropCardMove():Thread {
        ...
        if (_dropPExec == null || _dropPExec.state == ThreadState.TERMINATED) {
            ...
            for (var i:int = 0; i < tableArray.length; i++) {
                ...
                _dropPExec.addThread(new BeTweenAS3Thread(tableArray[i], {
                    x: getDropX(i),
                    y: getDropY(i),
                    scaleX: dropCardsScale(),
                    scaleY: dropCardsScale()
                }, null, 0.2, BeTweenAS3Thread.EASE_OUT_SINE, 0.0, true));
            }
            return _dropPExec;
        } else {
            return new Thread();
...
//            return new TweenerThread (acc, { x:distFoeP.x, y:distFoeP.y, delay:0.2, scaleX:0.8, scaleY:0.8, transition:"easeOutQuart", time: 0.5, hide: true} );
        disposeHandlers(acc);
        return new BeTweenAS3Thread(acc, {
            x: distFoeP.x,
            y: distFoeP.y,
            scaleX: 0.8,
            scaleY: 0.8
        }, null, 0.5, BeTweenAS3Thread.EASE_OUT_QUART, 0.2, false);
    }
...
class DealThread extends Thread {
    ...
    private function deal():void {
        ...
        var cardMoveTween:Thread = new BeTweenAS3Thread(_acc, {
            ...
            scaleX: _scale,
            scaleY: _scale
        }, null, _moveTime, BeTweenAS3Thread.EASE_OUT_SINE, _wait, true);
        var pExec:ParallelExecutor = new ParallelExecutor();
