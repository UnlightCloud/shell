...
//             _toolTipOwnerArray.psh([2,_foeDiceContainer]);
    ...
    public override function init():void {
        ...
        _plPoints.forEach(function (item:*, index:int, array:Array):void {
            _plPointContainer.addChild(item)
        });
        _foePoints.forEach(function (item:*, index:int, array:Array):void {
            _foePointContainer.addChild(item)
        });
...
//         }
    ...
    public function getHidePhaseThread(type:String = ""):Thread {
        ...
        if (_plCharaFace != null) {
            pExec.addThread(_plCharaFace.getHideThread())
        }
        ;
        if (_foeCharaFace != null) {
            pExec.addThread(_foeCharaFace.getHideThread())
        }
        ;
        _doneButton.buttonEnabled = false;
