    ...
    public function show():void {
        ...
        if (_viewType == VIEW_TYPE_LOBBY) {
            sExec.addThread(new ClousureThread(function ():void {
                _baseN.ok.visible = true;
            }));
        } else {
            sExec.addThread(new ClousureThread(function ():void {
                _base.ok.visible = true;
            }));
        }
            ...
            switch (bonus[i].treasureType) {
                ...
                case Const.TG_SLOT_CARD:
                    switch (bonus[i].slotType) {
                        case Const.SCT_EQUIP:
                            if (EquipCard.ID(bonus[i].treasureId)) {
                                name = EquipCard.ID(bonus[i].treasureId).name;
                            }
                            break;
                        case Const.SCT_EVENT:
                            if (EventCard.ID(bonus[i].treasureId)) {
                                name = EventCard.ID(bonus[i].treasureId).name;
                            }
                            break;
                        case Const.SCT_WEAPON:
                            if (WeaponCard.ID(bonus[i].treasureId)) {
                                name = WeaponCard.ID(bonus[i].treasureId).name;
                            }
                            break;
                        default:
                    ...
                    break;
                case Const.TG_AVATAR_ITEM:
                    if (AvatarItem.ID(bonus[i].treasureId)) {
                        name = AvatarItem.ID(bonus[i].treasureId).name;
                    }
                    break;
                case Const.TG_AVATAR_PART:
                    if (AvatarPart.ID(bonus[i].treasureId)) {
                        name = AvatarPart.ID(bonus[i].treasureId).name;
                    }
                    break;
                case Const.TG_GEM:
