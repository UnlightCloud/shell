...
public class DiceObject extends BaseObject {
    ...
    public function roll():Thread {
        ...
        var diceMoveTween:Thread = new BeTweenAS3Thread(_object, {
            x: Math.random() * 10 + (_length / 3) * 20,
            z: Math.random() * 10 + (_length % 3) * 20
        }, null, 0.5, BeTweenAS3Thread.EASE_OUT_SINE, _delay, true);

        // 1と6はY回転しない（行列より軽そうなのでこれで）
        if (_eye == 1 || _eye == 6) {
            diceRotTween = new BeTweenAS3Thread(_object, {
                rotationX: _rot[(_eye - 1) * 2],
                rotationZ: _rot[(_eye - 1) * 2 + 1]
            }, null, 0.5, BeTweenAS3Thread.EASE_IN_OUT_BOUNCE, _delay, true);
        } else {
            diceRotTween = new BeTweenAS3Thread(_object, {
                rotationX: _rot[(_eye - 1) * 2],
    ...
    public function destroy():void {
        if (_bmpMaterial != null) {
            if (_bmpMaterial.bitmap != null) {
                _bmpMaterial.bitmap.dispose()
            }
