...
//            _passiveInventory3MC.removeEventListener(MouseEvent.CLICK, passiveOpenHandler);
        ...
        _featInventory.forEach(function (item:*, index:int, array:Array):void {
            if (item) {
                item.removeEventListener(MouseEvent.CLICK, featOpenHandler)
            }
        });
        _passiveInventory.forEach(function (item:*, index:int, array:Array):void {
            if (item) {
                item.removeEventListener(MouseEvent.CLICK, featOpenHandler)
            }
        });
    ...
    public function addPassiveSkillContainer(name:String, caption:String):void {
        ...
        _passiveContainers.forEach(function (item:*, index:int, array:Array):void {
            if (item) {
                item.visible = false
            }
        });
    ...
    public function onTabAClick(e:MouseEvent):void {
        ...
        _featsContainers.forEach(function (item:*, index:int, array:Array):void {
            if (item) {
                item.visible = true
            }
        });
    ...
    private function setCompleteFeatNo():void {
        ...
        // キャプションの表示
        _featsCaption.forEach(function (item:*, index:int, array:Array):void {
            if (item) {
                item.visible = false
            }
        });
    ...
    private function setCompletePassiveNo():void {
        ...
        var pExec:SerialExecutor = new SerialExecutor()
        // キャプションの表示
        _passiveCaption.forEach(function (item:*, index:int, array:Array):void {
            if (item) {
                item.visible = false
            }
        });
