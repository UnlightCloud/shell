...
//            _container.addChild(_levelUpBase);
    ...
    private function showCards():void {
        ...
        _cccLow.forEach(function (item:*, index:int, array:Array):void {
            pExec.addThread(item.getRequirementsShowThread(_cardsContainer))
        });
...
//            pExec.addThread(_fade.getHideThread());
        ...
        _cccLow.forEach(function (item:*, index:int, array:Array):void {
            pExec.addThread(item.getRequirementsHideThread())
        });
        _cccHigh.forEach(function (item:*, index:int, array:Array):void {
            pExec.addThread(item.getRequirementsHideThread())
        });
        ...
        _labelLow.forEach(function (item:*, index:int, array:Array):void {
            _lockContainer.removeChild(item)
        });
        _labelLowNum.forEach(function (item:*, index:int, array:Array):void {
            _lockNumContainer.removeChild(item)
        });
    ...
    private function growth2():void {
        ...
        if (_lowId.length) {
            ...
            _cccLow.forEach(function (item:*, index:int, array:Array):void {
                pExec.addThread(item.getHideThread())
            });
            _labelLow.forEach(function (item:*, index:int, array:Array):void {
                _lockContainer.removeChild(item)
            });
